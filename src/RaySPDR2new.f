...
c        find dx, dy and dz for each step
         ...
         x1 = rr*dsin(ar)*dcos(br)
         y1 = rr*dsin(ar)*dsin(br)
         z1 = rr*dcos(ar)
         x2 = rs*dsin(as)*dcos(bs)
         y2 = rs*dsin(as)*dsin(bs)
         z2 = rs*dcos(as)
...
c     Check coordinates
      ...
      if(bbs.LT.0.OR.bbs.GT.360.)then
        write(*,*)'Longitude of source is out of range'
        stop
      endif
      if(bbr.LT.0.OR.bbr.GT.360.)then
...
c     write(*,*)sqrt((x2-x1)**2+(y2-y1)**2)
      do 155  j=1,ni+1
          ...
          y = y1 + dy*(j-1)
          z = z1 + dz*(j-1)                                             
          r(j) = dsqrt(x**2 + y**2 + z**2)                         
          acosa=z/r(j)
          if(acosa.LT.-1.)acosa=-1.
          if(acosa.GT.1)acosa=1.
          a(j) = acos(acosa)                                   
          acosa=x/r(j)/dsin(a(j))
          if(acosa.LT.-1.)acosa=-1.
          if(acosa.GT.1)acosa=1.
          b(j) = acos(acosa)
...
c              write(*,*)'Tut 1'
                  ...
                  a1 = a(k-1)                                 
                  b1 = b(k-1)                                 
                  x1 = r1*dsin(a1)*dcos(b1)                           
                  y1 = r1*dsin(a1)*dsin(b1)                           
                  z1 = r1*dcos(a1)                                   
                  r3 = r(k+1)                                 
                  a3 = a(k+1)                                 
                  b3 = b(k+1)                                 
                  x3 = r3*dsin(a3)*dcos(b3)                           
                  y3 = r3*dsin(a3)*dsin(b3)                           
                  z3 = r3*dcos(a3)                                   
                  dx = x3 - x1                                      
                  dy = y3 - y1                                      
                  dz = z3 - z1                                      
...
c                 first find the length of segment
                  dseg=dsqrt((dx/2)**2+(dy/2)**2+(dz/2)**2)
...
c                 write(*,*)vrd
                  ...
                  rvb = vb - vrd*pb
c                 write(*,*)rvr,rva,rvb
                  rvs = dsqrt(rvr*rvr + rva*rva + rvb*rvb)               
c                 write(*,*)rvs
c                 write(*,*)'posle rvs'
                  if(rvs .eq. 0.) then                              
                      r(k) = r2                                   
...
c                     write(*,*)'rcur ',rcur
                      rcur = -rcur + dsqrt(rcur**2+dn/(8.*cc*v2))     
c                     write(*,*)'rcur ',rcur
                      rdr = rvr * rcur
                      rda = rva * rcur
                      rdb = rvb * rcur
...
c
      ...
      do 100  i=1,n-1                                                   
          j  = i + 1                                                    
          xi = r(i)*dsin(a(i))*dcos(b(i))                                 
          yi = r(i)*dsin(a(i))*dsin(b(i))                                 
          zi = r(i)*dcos(a(i))                                           
          xj = r(j)*dsin(a(j))*dcos(b(j))                                 
          yj = r(j)*dsin(a(j))*dsin(b(j))                                 
          zj = r(j)*dcos(a(j))                                           
...
c                                                                       
      ...
      do 100  j=1,m-1
          x1 = r(j)*dsin(a(j))*dcos(b(j))
          y1 = r(j)*dsin(a(j))*dsin(b(j))
          z1 = r(j)*dcos(a(j))                                               
          x2 = r(j+1)*dsin(a(j+1))*dcos(b(j+1))
          y2 = r(j+1)*dsin(a(j+1))*dsin(b(j+1))
          z2 = r(j+1)*dcos(a(j+1))
          dl = (x1-x2)**2 + (y1-y2)**2 + (z1-z2)**2
          rv2 = 1./vel2(isp,r(j+1),a(j+1),b(j+1),-2)
          sm = (rv1 + rv2) / 2.         
          rtim = rtim + dsqrt(dl)*sm
          rv1 = rv2
  100 continue                                                          
      return                                                            
      end                                                               
...
C-------------------------------------------------------------------
      !the inconsistance of coordinate is really a pain in the ass,
      ! it took me two days to finally find where is the little bug
      ! added by Hongjiann @ MIT 2014/08/21
C-------------------------------------------------------------------
...
c       longl=xn(1)
c       longr=xn(nx)
c       if (longl.le.0) longl=longl+360
c       if (longr.le.0) longr=longr+360
C--------------------------------------------------------
...
c       if(lo.GT.longl+0.1.AND.lo.LT.longr-0.1.AND.la.GT.yn(1)+0.1
c     &   .AND.la.LT.yn(ny)-0.1)then
...
c  use Prothero's intmap here
	 ...
	 print*,lo,la
          print*,'warning:out of the region, change MOD'
          stop
...
3     continue
      ...
      th        = atan2(des,dhe)
      dtddel    = (dsin(th)*he/ve)
c     write(*,*)'rp= ',dtddel
      dtdr      = abs(dcos(th)/ve)
c     write(*,*)'az= ',az*r2d
      if(az*r2d.GT.360.)az=az-360.*dpi
      dth3 =-dtddel*dcos(az)
      dth4 = dtddel*dsin(az)*dsin(pe)
      dth2 =-dsqrt((he/ve)**2-dtddel**2)/he
      return
      end
...
c100	format(3(2f7.3,i3))
      !note there seems a bug here, xf yf are not at the same level as
      !zf, so this kind of interpolation is problematic.
      ! modified by Hongjian @ mit 2014/08/19
      xf=(x-xn(ip))/(xn(ip1)-xn(ip))
      yf=(y-yn(jp))/(yn(jp1)-yn(jp))
      zf=(z-zn(kp))/(zn(kp1)-zn(kp))
!      xf=(10000*(x-xn(ip)))/(10000*(xn(ip1)-xn(ip)))
!      yf=(10000*(y-yn(jp)))/(10000*(yn(jp1)-yn(jp)))
!      zf=(10*(z-zn(kp)))/(10*(zn(kp1)-zn(kp)))
      xf1=1.0-xf
      yf1=1.0-yf
      zf1=1.0-zf
!	if(xf<0.or.xf>1.or.yf<0.or.yf>1.or.zf<0.or.zf>1) then
!	print*,x,y,z,xn(ip),yn(jp),zn(kp),xn(ip1),yn(jp1),zn(kp1)
!	stop
!	endif
c
      wv(1)=xf1*yf1*zf1
      ...
      wv(7)=xf1*yf*zf
      wv(8)=xf*yf*zf
!      write(*,*)wv(1),wv(2),wv(3),wv(4),wv(5),wv(6),wv(7),wv(8)
...
c
         xnow=float(i)*bld-xl
         if (anint(xnow*10000).ge.anint(xn(ix1)*10000)) ix=ix1
c
         ixloc(i)=ix
 10   continue
...
c
         ynow=float(i)*bld-yl
         if (anint(ynow*10000).ge.anint(yn(iy1)*10000)) iy=iy1
c
         iyloc(i)=iy
 15   continue
c
...
c
         ...
         if (anint(znow*10).ge.anint(zn(iz1)*10)) iz=iz1
c
         izloc(i)=iz
 20   continue
c
