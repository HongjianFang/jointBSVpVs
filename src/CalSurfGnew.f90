...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!PROGRAM tomo_surf
subroutine CalSurfG(nx,ny,nz,nparpi,vel,iw,rw,col,dsurf,obst, &
...
!	variables defined by Hongjian Fang
	...
	real vpz(nz-1),vsz(nz-1),rhoz(nz-1),depz(nz-1)
       ...
       real*8 sen_rhoRg(nx*ny,kmaxRg,nz-1)
       real*8 sen_vsLc(nx*ny,kmaxLc,nz-1),sen_vpLc(nx*ny,kmaxLc,nz-1)
       real*8 sen_rhoLc(nx*ny,kmaxLc,nz-1)
       real*8 sen_vsLg(nx*ny,kmaxLg,nz-1),sen_vpLg(nx*ny,kmaxLg,nz-1)
       real*8 sen_rhoLg(nx*ny,kmaxLg,nz-1)
...
allocate(fdm(0:nvz+1,0:nvx+1))
             ...
             if(kmaxRc.gt.0) then
                 iwave=2
                 igr=0
                 call depthkernel(nx,ny,nz,vel,pvRc,sen_vsRc,sen_vpRc, &
                    sen_rhoRc,iwave,igr,kmaxRc,tRc,depz,minthk,maxnx,maxny,maxnz2)
             endif
             ...
             if(kmaxRg.gt.0) then
                 ...
                 call caldispersion(nx,ny,nz,vel,pvRc, &
                    iwave,igr,kmax,tRg,depz,minthk,maxnx,maxny,maxnz2)
                 igr=1
                 call depthkernel(nx,ny,nz,vel,pvRg,sen_vsRg,sen_vpRg, &
                    sen_rhoRg,iwave,igr,kmaxRg,tRg,depz,minthk,maxnx,maxny,maxnz2)
             endif

             if(kmaxLc.gt.0) then
                 iwave=1
                 igr=0
                 call depthkernel(nx,ny,nz,vel,pvLc,sen_vsLc,sen_vpLc, &
                    sen_rhoLc,iwave,igr,kmaxLc,tLc,depz,minthk,maxnx,maxny,maxnz2)
             endif
             ...
             if(kmaxLg.gt.0) then
                 ...
                 call caldispersion(nx,ny,nz,vel,pvLc, &
                    iwave,igr,kmax,tLg,depz,minthk,maxnx,maxny,maxnz2)
                 igr=1
                 call depthkernel(nx,ny,nz,vel,pvLg,sen_vsLg,sen_vpLg, &
                    sen_rhoLg,iwave,igr,kmaxLg,tLg,depz,minthk,maxnx,maxny,maxnz2)
             endif
...
!  
 ...
 count1=count1+1
dsurf(count1)=(cbst1-obst(count1))*1000 
        endif
!!-------------------------------------------------------------
!   ENDIF
!
...
!
      ...
      row=0
       do jj=1,nx-2
    	do kk=1,ny-2
    	if (abs(fdm(jj,kk)).gt.1e-3) then
    	coe_rho=(1.6612-0.4721*2*vel(jj+1,ny-kk,2:nz-1)+ &
        0.0671*3*vel(jj+1,ny-kk,2:nz-1)**2-0.0043*4*vel(jj+1,ny-kk,2:nz-1)**3+ &
        0.000106*5*vel(jj+1,ny-kk,2:nz-1)**4)
    	row((nvx-kk)*nvz+jj:(nzf-1)*nvz*nvx+(nvx-kk)*nvz+jj:nvx*nvz)= &
        (sen_rho(jj*(nvx+2)+kk+1,knum1(knumi),1:nzf)*coe_rho(1:nzf)+ &
        sen_vp(jj*(nvx+2)+kk+1,knum1(knumi),1:nzf))*fdm(jj,kk)
	    row(nparpi+(nvx-kk)*nvz+jj:nparpi+(nzf-1)*nvz*nvx+(nvx-kk)*nvz+jj:nvx*nvz)= &
        sen_vs(jj*(nvx+2)+kk+1,knum1(knumi),1:nzf)*fdm(jj,kk)
    	endif
	    enddo
	    enddo
                if(vpvsinv==0) then
		do nn=1,2*nparpi
		if(abs(row(nn)).gt.5e-2) then
		nar=nar+1
		rw(nar)=real(row(nn))
		!iw(nar+1)= count1
		iw(nar+1)= count11
		...
		enddo
                do nn=nparpi+1,2*nparpi
                jj=mdexfx(nn)         
                 k1=(jj-1)/nxy2+2
                 j1=2+(jj-1+(2-k1)*nxy2)/nx2
                 i1=1+jj+nx2*(2-j1)+nxy2*(2-k1)   
                 if(k1>=nz) k1=k1+2
		if(abs(row(nn)).gt.5e-2) then
		nar=nar+1
		rw(nar)=-real(row(nn))*vel(i1,j1,k1-nz)/vel(i1,j1,k1)**2
		iw(nar+1)= count11
              col(nar)=nn
		endif
		enddo
		...
		if(abs(row(nn)).gt.5e-2) then
		nar=nar+1
		rw(nar)=real(row(nn))*vel(i1,j1,k1-nz)/vel(i1,j1,k1)
		iw(nar+1)= count11
              col(nar)=nn-nparpi
		endif
		enddo
...
!fang!      WRITE(40)nrp
!fang!      DO j=1,nrp
!fang!         rayx=(pi/2-rgx(j))*180.0/pi
!fang!         rayz=rgz(j)*180.0/pi
!fang!         WRITE(40)rayx,rayz
!fang!      ENDDO
!fang!   ENDIF
!
...
subroutine synthetic(nx,ny,nz,nparpi,vel,obst, &
              goxdf,gozdf,dvxdf,dvzdf,kmaxRc,kmaxRg,kmaxLc,kmaxLg, &
              tRc,tRg,tLc,tLg,wavetype,igrt,periods,depz,minthk, &
              scxf,sczf,rcxf,rczf,nrc1,nsrcsurf1,knum1,kmax,nsrcsurf,nrcf, &
...
allocate(fdm(0:nvz+1,0:nvx+1))
             ...
             if(kmaxRg.gt.0) then
                 iwave=2
                 igr=1
                 call caldispersion(nx,ny,nz,vel,pvRg,&
                    iwave,igr,kmaxRg,tRg,depz,minthk,maxnx,maxny,maxnz2)
             endif
             ...
             if(kmaxLc.gt.0) then
                 ...
                 igr=0
                 call caldispersion(nx,ny,nz,vel,pvLc, &
                    iwave,igr,kmaxLc,tLc,depz,minthk,maxnx,maxny,maxnz2)
             endif

             if(kmaxLg.gt.0) then
                 iwave=1
                 igr=1
                 call caldispersion(nx,ny,nz,vel,pvLg, &
                    iwave,igr,kmaxLg,tLg,depz,minthk,maxnx,maxny,maxnz2)
             endif
...
obst(count1)=cbst1 + gaussian()*cbst1*noiselevel
!!-------------------------------------------------------------
!   ENDIF
!
!  Calculate raypath geometries and write to file if required.
